...
@file:OptIn(InternalAPI::class, Prerelease::class)
...
/**
 * Sets [Episode.date] (unix millis) from a date string. Accepts ISO-8601 ("2024-01-05",
 * "2024-01-05T12:30:00Z", "2024-01-05T14:35+02:00") or any pattern built from the tokens
 * yyyy / yy / MM / dd / HH / hh / mm / ss. Implemented without kotlinx-datetime and never throws.
 */
@Deprecated("Use addDate with a date string instead.", level = DeprecationLevel.WARNING)
...
fun Episode.addDate(date: String?, format: String = "yyyy-MM-dd") {
    ...
    this.date = parseDateToMillis(date.trim(), format)
}

fun parseDateToMillis(input: String, format: String = "yyyy-MM-dd"): Long? {
...
/** Walks the pattern and reads the digits found at the same offsets from the input. */
private fun parseWithPattern(input: String, format: String): Long? {
    var year = 1970
    var month = 1
    var day = 1
    var hour = 0
    ...
    while (fi < format.length) {
        val token = when {
            format.startsWith("yyyy", fi) -> "yyyy"
            format.startsWith("yy", fi) -> "yy"
            format.startsWith("MM", fi) -> "MM"
            format.startsWith("dd", fi) -> "dd"
            format.startsWith("HH", fi) || format.startsWith("hh", fi) -> "HH"
            format.startsWith("mm", fi) -> "mm"
            format.startsWith("ss", fi) -> "ss"
            else -> null
        }
        if (token == null) {
            fi++
        ...
        val num = Regex("^\\d{1,4}").find(input, ii.coerceAtMost(input.length)) ?: return null
        val value = num.value.toIntOrNull() ?: return null
        when (token) {
            "yyyy" -> year = value
            "yy" -> year = 2000 + value
            "MM" -> month = value
    ...
    return toMillis(year, month, day, hour, minute, second, null)
}
...
private fun toMillis(
    ...
    val tz = when {
        offset.isNullOrEmpty() -> TimeZone.getDefault()
        offset == "Z" -> TimeZone.getTimeZone("UTC")
        else -> {
            val sign = if (offset.startsWith("-")) -1 else 1
            val digits = offset.drop(1).replace(":", "")
            val oh = digits.substring(0, minOf(2, digits.length)).toIntOrNull() ?: 0
            val om = if (digits.length > 2) digits.substring(2).toIntOrNull() ?: 0 else 0
            TimeZone.getTimeZone(
                "GMT" + (if (sign < 0) "-" else "+") + "%02d:%02d".format(oh, om)
            )
        }
    }
...
fun isUpcoming(dateString: String?): Boolean {
    val time = parseDateToMillis(dateString ?: return false) ?: return false
