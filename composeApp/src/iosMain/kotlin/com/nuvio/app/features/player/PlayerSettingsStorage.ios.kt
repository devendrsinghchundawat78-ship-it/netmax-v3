...
actual object PlayerSettingsStorage {
    ...
    actual fun loadSubtitleCustomFontPath(): String? {
        return NSUserDefaults.standardUserDefaults.stringForKey(subtitleCustomFontPathKey)
    }

    actual fun saveSubtitleCustomFontPath(path: String?) {
        val defaults = NSUserDefaults.standardUserDefaults
        if (path.isNullOrBlank()) {
            defaults.removeObjectForKey(subtitleCustomFontPathKey)
        } else {
            defaults.setObject(path, forKey = subtitleCustomFontPathKey)
        }
    }
