...
internal fun defaultNuvioThemeTokens(
    ...
    val background = if (darkTheme && amoled) Color.Black else if (darkTheme) palette.background else Color(0xFFF6F7F9)
    val textPrimary = if (darkTheme) Color(0xFFF5F7F8) else Color(0xFF17191D)
    val textSecondary = if (darkTheme) Color(0xFFB8BEC5) else Color(0xFF4F5660)
    val textMuted = if (darkTheme) Color(0xFF969CA3) else Color(0xFF69717B)
    val surface = if (darkTheme) palette.backgroundElevated else Color(0xFFFFFFFF)
    val surfaceCard = if (darkTheme) palette.backgroundCard else Color(0xFFECEEF2)
    val accent = palette.secondary
    val accentStrong = palette.secondaryVariant
    val borderSubtle = (if (darkTheme) Color(0xFF252A2A) else Color(0xFFD0D5DC)).copy(alpha = 0.55f)
    val borderDefault = if (darkTheme) Color(0xFF252A2A) else Color(0xFFD0D5DC)
    ...
    return NuvioThemeTokens(
        colors = NuvioColorTokens(
            ...
            borderStrong = if (darkTheme) Color(0xFF3A4040) else Color(0xFFB9C0C9),
            ...
            overlayHover = (if (darkTheme) Color.White else Color.Black).copy(alpha = NuvioTokens.Opacity.hover),
            overlayPressed = (if (darkTheme) Color.White else Color.Black).copy(alpha = NuvioTokens.Opacity.pressed),
            overlaySelected = (if (darkTheme) Color.White else Color.Black).copy(alpha = NuvioTokens.Opacity.selected),
            overlayDisabled = (if (darkTheme) Color.Black else Color.White).copy(alpha = NuvioTokens.Opacity.disabled),
            shimmer = (if (darkTheme) Color.White else Color.Black).copy(alpha = 0.10f),
            skeleton = (if (darkTheme) Color.White else Color.Black).copy(alpha = 0.06f),
